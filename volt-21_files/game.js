...
//  Version: 1.5.3 (TOTAL RESTORATION + MQTT)
// ═══════════════════════════════════════════════
console.log('%c[VOLTAGE 21] Version 1.5.3 loaded', 'color:#00ffff; font-weight:bold; font-size:1.4em;');
...
async function initMQTT(code) {
    ...
    online.client.onMessageArrived = (msg) => {
        ...
        if (online.isHost) handleGuestMessage(data); else handleHostMessage(data);
    };
...
function sendNet(data) {
    ...
    const msg = new Paho.MQTT.Message(JSON.stringify({ sender: online.clientId, ...data }));
    msg.destinationName = online.topic; online.client.send(msg);
}

function sendToGuest(data) { sendNet(data); }
function sendToHost(data)  { sendNet(data); }
...
function applyNetState(s) {
    ...
    updateUI();
    if (!online.isHost) {
        renderHand(state.p2.hand, true);
        renderOppHand(state.p1.hand, false);
    }
}
...
function showLobby() {
    return new Promise(resolve => {
        ...
        document.getElementById('online-back-btn').onclick = () => {
            ...
            if (online.client) { online.client.disconnect(); online.client = null; }
        };
        ...
        document.getElementById('create-room-btn').onclick = async () => {
            ...
            try {
                ...
                document.getElementById('lobby-status').textContent = 'Connection Error.';
            }
        };
        ...
        document.getElementById('join-room-btn').onclick = async () => {
            ...
            try {
                await initMQTT(code);
                sendToHost({ type: 'guestJoined' });
                
                const joinWait = new Promise((ok, fail) => {
                    online._resolvers.join = ok;
                ...
                document.getElementById('lobby-status').textContent = 'Host not found.';
            }
        };
    });
}
